# Infinite Stack in Storage (Fabric 1.20.1)
...
* **Storage** = every container built on vanilla's lootable-container base class: chests (incl. double and trapped),
  ...
  Hoppers, dispensers and droppers are excluded. Configurable in `config/infinitestack.json`:
...
* Counts over 127 are saved in an extra `infinitestack:count` NBT tag. If the mod is removed the world still loads; each oversized stack falls back to one normal stack.
...
Output: `build/libs/infinite-stack-in-storage-<version>.jar`. `./gradlew runClient` / `runServer` start a dev instance.
...
The same checks are packaged as a `/sstest [block id]` command in `infinite-stack-in-storage-<version>-gametest.jar` (built by
...
## How it works (src/main/java/dev/kkayam/infinitestack)
